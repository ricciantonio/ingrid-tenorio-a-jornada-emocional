      ...
      <div className="mx-auto max-w-6xl px-5 sm:px-8">
        <div className="grid gap-10 md:grid-cols-3">
          <div>
            ...
            <p className="mt-2 text-sm uppercase tracking-[0.2em] text-muted-foreground">Terapeuta Emocional</p>
            <div className="gold-divider mt-5 w-20" />
            <p className="mt-5 max-w-xs text-sm text-foreground/70">
              Acompanhamento emocional especializado para quem busca clareza, identidade e poder pessoal.
            </p>
          </div>
          ...
          <div>
            ...
            <ul className="mt-5 space-y-2 text-sm">
              {[
                ...
                <li key={l.href}>
                  <a href={l.href} className="text-foreground/70 transition-colors hover:text-primary">
                    {l.label}
                  </a>
                </li>
              ))}
        ...
        <div className="mt-14 flex flex-col items-start justify-between gap-3 border-t border-border pt-6 text-xs text-muted-foreground sm:flex-row sm:items-center">
          <p>© {new Date().getFullYear()} Ingrid Tenorio · Todos os direitos reservados.</p>
          <p className="italic">Atendimentos confidenciais e em respeito ao sigilo profissional.</p>
        </div>
      </div>
    </footer>
  );
